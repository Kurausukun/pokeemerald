...
{
    if ((gMPlayInfo_BGM.status & MUSICPLAYER_STATUS_PAUSE) && (gMPlayInfo_BGM2.status & MUSICPLAYER_STATUS_PAUSE))
        return TRUE;
    if (!((gMPlayInfo_BGM.status & MUSICPLAYER_STATUS_TRACK)) && !(gMPlayInfo_BGM2.status & MUSICPLAYER_STATUS_TRACK))
        return TRUE;
    return FALSE;
}
...
{
    if (gMain.inBattle)
        m4aMPlayVolumeControl(&gMPlayInfo_BGM2, 0xFFFF, 85);
    else
