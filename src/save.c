 ...
 * Sector 14:           Hall of Fame
 * Sector 15:           Recorded Battle
 *
...
struct
{
    u16 offset;
    u16 size;
} static const sSaveSlotLayout[NUM_MAIN_SECTORS] =
{
...
COMMON_DATA struct SaveSectorLocation gRamSaveSectorLocations[NUM_MAIN_SECTORS] = {0};
...
static u8 WriteSaveSectorOrSlot(u16 sectorId, const struct SaveSectorLocation *locations)
{
    ...
    else
    {
        ...
        gLastWrittenSector++;
        gLastWrittenSector = gLastWrittenSector % NUM_MAIN_SECTORS;
        gSaveCounter++;
        status = SAVE_STATUS_OK;

        for (i = 0; i < NUM_MAIN_SECTORS; i++)
...
static u32 RestoreSaveBackupVarsAndIncrement(const struct SaveSectorLocation *locations)
{
    ...
    gLastSaveCounter = gSaveCounter;
    gLastWrittenSector++;
    gLastWrittenSector %= NUM_MAIN_SECTORS;
    gSaveCounter++;
...
static u8 GetSaveValidStatus(const struct SaveSectorLocation *locations)
{
    ...
    if (signatureValid)
    {
        if (validSectorFlags == (1 << NUM_MAIN_SECTORS) - 1)
            return SAVE_STATUS_OK;
        else
            return SAVE_STATUS_ERROR;
    }
...
{
    u8 status = HandleWriteIncrementalSector(NUM_MAIN_SECTORS, gRamSaveSectorLocations);
    if (gDamagedSaveSectors)
...
{
    HandleReplaceSectorAndVerify(NUM_MAIN_SECTORS, gRamSaveSectorLocations);
    if (gDamagedSaveSectors)
        DoSaveFailedScreen(SAVE_NORMAL);
    return FALSE;
}
...
{
    CopySectorSignatureByte(NUM_MAIN_SECTORS, gRamSaveSectorLocations);
    if (gDamagedSaveSectors)
        DoSaveFailedScreen(SAVE_NORMAL);
    return FALSE;
}
